...
public class Estoques extends JDialog {
	...
	private JTextField txtValorDig;
	private JLabel lblFoto;
	private JButton brnCarregar;
	...
	public Estoques() {
		...
		setTitle("Marvi Cold - ESTOQUE");
		setBounds(100, 100, 973, 450);
		...
		txtValor.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)),
				"VALOR TOTAL", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		txtValor.setBounds(562, 293, 116, 42);
		...
		btnUpdate.setBounds(98, 345, 52, 52);
		...
		btnDelete.setBounds(156, 345, 52, 52);
		...
		btnLimpar.setBounds(272, 345, 52, 52);
		...
		btnCreate.setBounds(37, 345, 52, 52);
		...
		txtLucro.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "LUCRO",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		txtLucro.setBounds(450, 293, 102, 42);
		getContentPane().add(txtLucro);
		txtValorDig = new JTextField();
		txtValorDig.addKeyListener(new KeyAdapter() {
			@Override
			public void keyPressed(KeyEvent e) {
				totalValor();
			}
		...
		txtValorDig.setToolTipText("");
		txtValorDig.setColumns(10);
		txtValorDig.setBorder(new TitledBorder(
				new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "VALOR",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		txtValorDig.setBounds(338, 293, 102, 42);
		getContentPane().add(txtValorDig);
		lblFoto = new JLabel("");
		lblFoto.setBorder(new BevelBorder(BevelBorder.LOWERED, null, null, null, null));
		lblFoto.setIcon(new ImageIcon(Estoques.class.getResource("/img/imagem_aqui.png")));
		lblFoto.setBounds(688, 22, 256, 256);
		getContentPane().add(lblFoto);
		brnCarregar = new JButton("CARREGAR FOTO");
		brnCarregar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				carregarFoto();
			}
		});
		brnCarregar.setForeground(Color.GRAY);
		brnCarregar.setBounds(688, 293, 256, 21);
		getContentPane().add(brnCarregar);
	}
			...
			if (linha1 >= 0) {
				...
				try {
					...
					if (rs.next()) {
						...
						Icon foto = new ImageIcon(icone.getImage().getScaledInstance(lblFoto.getWidth(), getHeight(),
								Image.SCALE_SMOOTH));
						lblFoto.setIcon(foto);
						txtValorDig.setText(rs.getString(14));
		...
		try {
			...
			if (rs.next()) {
				...
				Icon foto = new ImageIcon(
						icone.getImage().getScaledInstance(lblFoto.getWidth(), getHeight(), Image.SCALE_SMOOTH));
				lblFoto.setIcon(foto);
				txtValorDig.setText(rs.getString(14));
				txtLucro.setText(rs.getString(15));
				String setarData = rs.getString(12);
	...
	private void adicionarItem() {
		if (txtIDforn.getText().isEmpty()) {
			...
			JOptionPane.showMessageDialog(null, "Adicione uma foto");
		} else {
			String create = "insert into estoques(idforn,produto,codebarra,unimedida,localarm,estoque,estoquemin,descricao,validade,valor,foto,valordig,lucro) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
			try {
				con = dao.conectar();
				...
				pst.setBlob(11, fis, tamanho);
				pst.setString(12, txtValorDig.getText());
	...
	private void editarItem() {
		if (txtIDforn.getText().isEmpty()) {
			...
			if (fotoCarregada == true) {
				String update = "update estoques set idforn=?,produto=?,codebarra=?,unimedida=?,localarm=?,estoque=?,estoquemin=?,descricao=?,validade=?,valor=?,foto=?,valordig=?,lucro=? where idprodut=?";
				try {
					con = dao.conectar();
				...
				String update = "update estoques set idforn=?,produto=?,codebarra=?,unimedida=?,localarm=?,estoque=?,estoquemin=?,descricao=?,validade=?,valor=?,valordig=?,lucro=? where idprodut=?";
				try {
					con = dao.conectar();
					pst = con.prepareStatement(update);
					pst.setString(1, txtIDforn.getText());
					...
					pst.setString(8, txtDescricao.getText());
					pst.setString(10, txtValor.getText());
					pst.setString(11, txtValorDig.getText());
	...
	private void totalValor() {
		if (!txtValorDig.getText().equals("0") || !txtLucro.getText().equals("0")) {
			double totalvalor01 = Double.valueOf(txtValorDig.getText()).doubleValue();
			double totalvalor02 = Double.valueOf(txtLucro.getText()).doubleValue() / 100.00;
			double resultado = totalvalor01 + (totalvalor02 * totalvalor01);
			String resultadoValor = Double.toString(resultado);
			txtValor.setText(resultadoValor);
		} else {
