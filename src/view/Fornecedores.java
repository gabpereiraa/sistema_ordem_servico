...
public class Fornecedores extends JDialog {
	...
	public Fornecedores() {
		...
		setTitle("Marvi Cold - FORNECEDORES");
		...
		lblNewLabel_2_2.setBounds(10, 210, 74, 14);
		...
		txtCnpj.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				if ( txtCnpj.getText().length() == 14 ) {
			        ...
			        valido = 8;
			        txtCnpj.setBorder(new LineBorder(Color.GRAY));
					lbltxtCNPJ.setVisible(false);
			    }else if ( txtCnpj.getText().length() >= 12 || txtCnpj.getText().length() <= 13) {
			        valido = 8;
			        txtCnpj.setBorder(new LineBorder(Color.GRAY));
			        lbltxtCNPJ.setVisible(false);
			    } else {
		...
		btnDelete.setBounds(152, 357, 52, 52);
		...
		btnUpdate.setBounds(88, 357, 52, 52);
		...
		btnCreate.setBounds(24, 357, 52, 52);
		...
		lblNewLabel_6.setBounds(10, 187, 74, 14);
		...
		lblNewLabel_10.setBounds(10, 236, 96, 14);
		contentPanel.add(lblNewLabel_10);
		lblNewLabel_11 = new JLabel("");
		lblNewLabel_11.setToolTipText("Digite a Razão para iniciar a pesquisa pela Razão");
		lblNewLabel_11.setIcon(new ImageIcon(Fornecedores.class.getResource("/img/search-marvi.png")));
		lblNewLabel_11.setBounds(359, 59, 32, 32);
		contentPanel.add(lblNewLabel_11);
		txtIDforn = new JTextField();
		txtIDforn.setEditable(false);
		txtIDforn.setBorder(new TitledBorder(null, "ID Fornecedor", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		lbltxtCNPJ = new JLabel("CNPJ Inválido");
		lbltxtCNPJ.setVisible(false);
		lbltxtCNPJ.setForeground(new Color(255, 0, 0));
		lbltxtCNPJ.setFont(new Font("Arial", Font.BOLD, 12));
		lbltxtCNPJ.setBounds(381, 103, 96, 14);
		contentPanel.add(lbltxtCNPJ);
	}
	...
	private void buscarClienteLista() {
		if (txtRazao.getText().isEmpty()) {
			...
			if (linha >= 0) {
				String read2 = "select * from fornecedores where razao like '" + txtRazao.getText() + "%'" + "order by razao limit " + (linha) + " , 1";
				try {
					con = dao.conectar();
				...
				pst.setString(14, txtIDforn.getText());
				 if ( txtCnpj.getText().length() == 14 ) {
					    ...
					    if (ValidaCNPJ.isCNPJ(CNPJ) == true) {
					    	...
					    	JOptionPane.showMessageDialog(null, "Preencha um CNPJ Válido");
					    	txtCnpj.requestFocus();			
			            }
				    } else {
				    	JOptionPane.showMessageDialog(null, "Preencha umc CNPJ Valido");
				    	txtCnpj.requestFocus();
				    }
				pst.executeUpdate();
	...
	private void validadordeCNPJ() {
		...
		CNPJ = txtCnpj.getText();
		if (ValidaCNPJ.isCNPJ(CNPJ) == true) {
			txtCnpj.setBorder(new LineBorder(new Color(0, 255, 0)));
			StringBuilder mensagem = new StringBuilder();
			lbltxtCNPJ.setVisible(false);
			valido = 1;
		} else {
			txtCnpj.setBorder(new LineBorder(new Color(255, 0, 0)));
			StringBuilder mensagem = new StringBuilder();
			valido = 4;
			lbltxtCNPJ.setVisible(true);
		}
	}
